...
class Session(std.Controller):
	...
	def fetch(self):
		return self.model().find(join = [('User', 'user_id')], start_from = self.get_input('start'), limit = self.get_input('limit')).fetch(fields_mask = [('user_name', 'user'), ('token', 'token'), ('created_at', 'access')])